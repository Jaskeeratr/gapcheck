...
def fetch_remoteok_jobs(max_items: int = 120, student_only: bool = True) -> list[NormalizedJobListing]:
    ...
    for row in rows[:max_items]:
        ...
        external_id = f"remoteok:{row.get('id') or row.get('slug') or title.lower().replace(' ', '-')}"
        apply_url = str(row.get("url") or "").strip() or None
        posted_at = _to_datetime(row.get("epoch") or row.get("date"))
